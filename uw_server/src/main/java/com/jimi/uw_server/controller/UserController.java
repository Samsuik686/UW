...
public class UserController extends Controller {
	...
	public void login(String uid, String password) {
		/*
		 * User user = userService.login(uid, password); // 判断是否重复登录 String tokenId =
		 * getPara(TokenBox.TOKEN_ID_KEY_NAME); if (tokenId != null) { User user2 =
		 * TokenBox.get(tokenId, SESSION_KEY_LOGIN_USER); if (user2 != null &&
		 * user.getUid().equals(user2.getUid())) { throw new
		 * ParameterException("请勿重复登录！"); } }else { tokenId = TokenBox.createTokenId();
		 * user.put(TokenBox.TOKEN_ID_KEY_NAME, tokenId); if (user.getType().equals(5))
		 * { UWTokenBox.putUWToken(tokenId, user.getUid(), SESSION_KEY_LOGIN_USER, user,
		 * true);
		 * 
		 * }else { UWTokenBox.putUWToken(tokenId, user.getUid(), SESSION_KEY_LOGIN_USER,
		 * user, false); } } renderJson(ResultUtil.succeed(user));
		 */

		User user = userService.login(uid, password); // 判断是否重复登录
	...
	public void update(String uid, String name, String password, Boolean enabled, Integer type) {
		 ...
		 * renderJson(ResultUtil.succeed()); User user = User.dao.findById(uid); //
		 * 如果禁用了某个用户 if (!user.getEnabled()) { String tokenId =
		 * getPara(TokenBox.TOKEN_ID_KEY_NAME); User user1 = TokenBox.get(tokenId,
		 * SESSION_KEY_LOGIN_USER); // 如果禁用的是用户自身 if
		 * (user.getUid().equals(user1.getUid())) { UWTokenBox.remove(tokenId); } } }
		 * else { renderJson(ResultUtil.failed()); }
		 */

		if (userService.update(uid, name, password, enabled, type)) {
			renderJson(ResultUtil.succeed());
			User user = User.dao.findById(uid); // 如果禁用了某个用户
			if (!user.getEnabled()) {
				String tokenId = getPara(TokenBox.TOKEN_ID_KEY_NAME);
				User user1 = TokenBox.get(tokenId, SESSION_KEY_LOGIN_USER); // 如果禁用的是用户自身
	...
	public void logout() {
		/*
		 * // 判断是否未登录 String tokenId = getPara(TokenBox.TOKEN_ID_KEY_NAME); User user =
		 * TokenBox.get(tokenId, SESSION_KEY_LOGIN_USER); if (user == null) { throw new
		 * ParameterException("没有用户登录，不需要退出登录！"); } UWTokenBox.remove(tokenId);
		 * renderJson(ResultUtil.succeed());
		 */
		// 判断是否未登录
		String tokenId = getPara(TokenBox.TOKEN_ID_KEY_NAME);
