...
public class ExceptionHandler {
	...
	public static void handleLoadException(String message) {
		 ...
		 * missiongroupid 包含“:”表示为出入库任务 String groupid = missionGroupId.split("_")[0];
		 * // missiongroupid 包含“:”表示为出入库任务 if (groupid.contains(":")) {
		 * for(AGVIOTaskItem item :
		 * TaskItemRedisDAO.getIOTaskItems(Integer.valueOf(groupid.split(":")[2]))) {
		 * if(item.getGroupId().equals(groupid)) { //把指定叉车的取空异常置为真
		 * RobotInfoRedisDAO.setloadException(item.getRobotId()); break; } } } else if
		 * (groupid.contains("@")) { // missiongroupid 包含“@”表示为盘点任务
		 * for(AGVInventoryTaskItem item1 :
		 * TaskItemRedisDAO.getInventoryTaskItems(Integer.valueOf(groupid.split("@")[1])
		 * )) { if(item1.getGroupId().equals(groupid)) { //把指定叉车的取空异常置为真
		 * RobotInfoRedisDAO.setloadException(item1.getRobotId());
		 * 
		 * break; } } }else if (groupid.contains("#")) { // missiongroupid 包含“@”表示为盘点任务
		 * for(AGVInventoryTaskItem item1 :
		 * TaskItemRedisDAO.getInventoryTaskItems(Integer.valueOf(groupid.split("#")[1])
		 * )) { if(item1.getGroupId().equals(groupid)) { //把指定叉车的取空异常置为真
		 * RobotInfoRedisDAO.setloadException(item1.getRobotId());
		 * 
		 * break; } } }else { // missiongroupid 不包含“:”表示为建仓任务 for(AGVBuildTaskItem item2
		 * : TaskItemRedisDAO.getBuildTaskItems()) {
		 * if(item2.getGroupId().equals(groupid)) { //把指定叉车的取空异常置为真
	...
	public static void handleDelMissionException(String message) {
		...
		if (groupid.contains(":") && missionGroupId.contains("B")) {
			...
			for (AGVIOTaskItem item : IOTaskRedisDAO.getIOTaskItems(Integer.valueOf(groupid.split(":")[2]))) {
				if (item.getGroupId().equals(groupid) && item.getState() > TaskItemState.ARRIVED_WINDOW) {
					...
					if (agvioTaskItem.getState() == TaskItemState.START_BACK) {
						TaskUtilsRedisDAO.delLocationStatus(agvioTaskItem.getWindowId(), agvioTaskItem.getGoodsLocationId());
					}
					break;
				}
					...
					if (item.getBoxId().equals(agvioTaskItem.getBoxId()) && item.getWindowId().equals(agvioTaskItem.getWindowId()) && item.getGoodsLocationId().equals(agvioTaskItem.getGoodsLocationId()) && item.getState() > TaskItemState.ARRIVED_WINDOW) {
						...
						if (item.getIsForceFinish().equals(false) && task.getType().equals(TaskType.OUT)) {
							...
							if (remainderQuantity <= 0) {
								...
								IOTaskRedisDAO.updateIOTaskItemInfo(item, TaskItemState.LACK, null, null, null, null, true, null);
							} else {
								IOTaskRedisDAO.updateIOTaskItemInfo(item, TaskItemState.FINISH_BACK, null, null, null, null, null, null);
							}
						} else {
							IOTaskRedisDAO.updateIOTaskItemInfo(item, TaskItemState.FINISH_BACK, null, null, null, null, null, null);
						}
						...
						materialBox.update();
						if (item.getIsCut()) {
							IOTaskRedisDAO.updateIOTaskItemInfo(item, TaskItemState.FINISH_CUT, null, null, null, null, null, false);
						}
						if (!item.getIsForceFinish()) {
							// 如果是出库任务，若实际出库数量小于计划出库数量，则将任务条目状态回滚到未分配状态
							if (task.getType() == TaskType.OUT) {
								IOTaskRedisDAO.updateIOTaskItemInfo(item, TaskItemState.WAIT_ASSIGN, 0, 0, 0, 0, null, null);
							} else { // 如果是入库或退料入库任务，若实际入库或退料入库数量小于计划入库或退料入库数量，则将任务条目状态回滚到等待扫码状态
								IOTaskRedisDAO.updateIOTaskItemInfo(item, TaskItemState.WAIT_SCAN, 0, 0, 0, 0, null, null);
							}
						}
			...
			for (AGVInventoryTaskItem item : InvTaskRedisDAO.getInventoryTaskItems(Integer.valueOf(groupid.split(":")[1]))) {
				...
				if (item.getGroupId().equals(groupid) && item.getState() > TaskItemState.ARRIVED_WINDOW) {
					if (item.getState() == TaskItemState.START_BACK) {
						TaskUtilsRedisDAO.delLocationStatus(item.getWindowId(), item.getGoodsLocationId());
					}
					MaterialBox materialBox = MaterialBox.dao.findById(item.getBoxId());
					materialBox.setIsOnShelf(true);
					materialBox.update();
			...
			for (AGVBuildTaskItem item2 : BulidTaskRedisDAO.getBuildTaskItems()) {
				if (item2.getGroupId().equals(groupid) && item2.getState() > BuildTaskItemState.WAIT_MOVE) {
					// 清除掉对应的建仓任务条目
					BulidTaskRedisDAO.removeBuildTaskItemByBoxId(item2.getBoxId().intValue());
