...
public class BuildHandler {
	...
	public static void sendBuildCmd(AGVBuildTaskItem item, MaterialBox materialBox) throws Exception {
		...
		// 更新任务条目状态为1
		BuildTaskItemDAO.updateBuildTaskItemState(item, TaskItemState.ASSIGNED);
	}
	...
	private static void handleStatus0(AGVStatusCmd statusCmd) {
		...
		// 匹配groupid
		for (AGVBuildTaskItem item : BuildTaskItemDAO.getBuildTaskItems()) {
			if (groupid.equals(item.getGroupId())) {
				// 更新buildTsakitems里对应item的robotid
				BuildTaskItemDAO.updateBuildTaskItemRobot(item, statusCmd.getRobotid());
			}
		}
	}
	...
	private static void handleStatus2(AGVStatusCmd statusCmd) throws Exception {
		...
		for (AGVBuildTaskItem item : BuildTaskItemDAO.getBuildTaskItems()) {
			if (groupid.equals(item.getGroupId())) {
				...
				BuildTaskItemDAO.updateBuildTaskItemState(item, BuildTaskItemState.FINISH_ONE_BUILD_TASK);
	...
	*/
	public static void clearTil(String srcPosition) {
		...
		if (isAllFinish) {
			BuildTaskItemDAO.removeBuildTaskItemBySrcPosition(srcPosition);
		}
	}
