...
public class RobotService extends SelectService {
	...
	public String back(Integer id, Boolean isLater, Integer state, User user) throws Exception {
		...
		synchronized (RegularTaskLock.IO_TASK_BACK_LOCK) {
			...
			Window window = Window.dao.findById(agvioTaskItem.getWindowId());
			if (window.getAuto() && task.getType().equals(TaskType.OUT)) {
				urOutTaskBack(agvioTaskItem.getId(), agvioTaskItem);
				return resultString;
			}
			...
			materialBox.setStatus(state).setUpdateTime(new Date());
			materialBox.update();
			if (materialBox != null && materialBox.getType().equals(MaterialBoxType.STANDARD)) {
				...
				synchronized (Lock.IO_TASK_REDIS_LOCK) {
					...
					if ((sameBoxItem == null) || (task.getType() != TaskType.OUT && unusedcapacity <= 0)) {
						...
						IOTaskItemRedisDAO.updateIOTaskItemInfo(sameBoxItem, TaskItemState.ARRIVED_WINDOW, agvioTaskItem.getWindowId(), agvioTaskItem.getGoodsLocationId(), agvioTaskItem.getBoxId(),
			...
			} else if (materialBox != null && materialBox.getType().equals(MaterialBoxType.NONSTANDARD)) {
				...
				synchronized (Lock.IO_TASK_REDIS_LOCK) {
					...
					if ((sameBoxItem == null) || (task.getType() != TaskType.OUT && materialBox.getStatus().equals(BoxState.FULL))) {
						...
						IOTaskItemRedisDAO.updateIOTaskItemInfo(sameBoxItem, TaskItemState.ARRIVED_WINDOW, agvioTaskItem.getWindowId(), agvioTaskItem.getGoodsLocationId(), agvioTaskItem.getBoxId(),
								agvioTaskItem.getRobotId(), null, null, null, null, null);
						// 更新任务条目绑定的叉车id
						resultString = "料盒中还有其他需要出库的物料，叉车暂时不回库！";
					}
				}
			...
			if (!isLater || task.getState().equals(TaskState.CANCELED)) {
				...
				IOTaskItemRedisDAO.updateIOTaskItemInfo(agvioTaskItem, TaskItemState.START_BACK, null, null, null, null, true, null, null, null, null);
		...
		synchronized (Lock.UR_OUT_TASK_LOCK) {
			...
			synchronized (Lock.IO_TASK_REDIS_LOCK) {
				...
				if (sameBoxItem == null) {
					...
					if (goodsLocation != null) {
						ioTaskHandler.sendBackLL(agvioTaskItem, materialBox, goodsLocation, task.getPriority());
						UrTaskInfoDAO.removeUrMaterialInfosByTaskAndBox(task.getId(), materialBox.getId());
					} else {
						throw new OperationException("找不到目的货位，仓口：" + agvioTaskItem.getWindowId() + "货位：" + agvioTaskItem.getGoodsLocationId());
					}
				} else { // 否则，将同料盒号、未被分配任务的任务条目状态更新为已到达仓口
					...
					// 发送到站包
					ForkliftReachPackage pack = new ForkliftReachPackage(agvioTaskItem.getTaskId(), agvioTaskItem.getBoxId());
			...
			if (isFinish) {
				...
				packingListItem.setFinishTime(new Date());
				packingListItem.update();
				List<Material> materials = Material.dao.find(IOTaskSQL.GET_MATERIAL_BY_PACKING_LIST_ITEM_ID, packingListItemId);
	...
	public AGVIOTaskItem getSameBoxItem(AGVIOTaskItem item, Task task) {
		for (AGVIOTaskItem item1 : IOTaskItemRedisDAO.getIOTaskItems(item.getTaskId())) {
