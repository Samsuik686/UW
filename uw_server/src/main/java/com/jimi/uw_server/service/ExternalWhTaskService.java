...
public class ExternalWhTaskService {
	 ...
	 * @return
	 */
	public String importTask(File file, Integer supplierId, Integer sourceHwId, Integer destinationHwId, User user) {
		...
		try {
			...
			Task task = new Task();
			task.setCreateTime(date).setDestination(destinationHwId).setSupplier(supplierId).setFileName(fileName).setType(TaskType.EXTERNAL_IN_OUT).setState(TaskState.FINISHED);
			if (sourceHwId == 0) {
	...
	public String importWastageTask(File file, Integer supplierId, Integer HwId, User user) {
		...
		try {
			...
			Task task = new Task();
			task.setCreateTime(date).setDestination(HwId).setSupplier(supplierId).setFileName(fileName).setType(TaskType.WASTAGE).setState(TaskState.FINISHED);
			task.save();
			for (ExternalWhLog externalWhLog : externalWhLogs) {
				externalWhLog.setTaskId(task.getId());
				externalWhLog.setTime(new Date());
	 ...
	 * @return
	 */
	public String addWorstageLog(Integer materialTypeId, Integer HwId, Integer quantity, User user ) {
