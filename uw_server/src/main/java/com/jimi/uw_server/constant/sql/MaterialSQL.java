...
public class MaterialSQL {
	...
	// 获取普通仓物料报表
	public static final String GET_REGUALR_MATERIAL_REPORT_SQL = "SELECT material_type.id as id, material_type.`no` as `no`, material_type.specification as specification,  SUM(material.remainder_quantity) AS quantity FROM material INNER JOIN material_type ON material_type.id = material.type WHERE material_type.type = ? AND material_type.enabled = 1 AND material.company_id = ? AND material_type.supplier = ? GROUP BY material.type ORDER BY material.type ASC";
	// 获取贵重仓物料报表
	public static final String GET_PRECIOUS_MATERIAL_REPORT_SQL = "SELECT material_type.id as id, material_type.`no` as `no`, material_type.specification as specification, SUM(material.remainder_quantity) AS quantity, material_type.designator FROM material INNER JOIN material_type ON  material_type.id = material.type WHERE material_type.type = ? AND material_type.enabled = 1 AND material.company_id = ? AND material_type.supplier = ? GROUP BY material.type ORDER BY  material.type ASC";
