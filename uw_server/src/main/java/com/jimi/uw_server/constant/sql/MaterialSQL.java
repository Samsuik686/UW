...
public class MaterialSQL {
	...
	//获取普通仓物料报表
	public static final String GET_REGUALR_MATERIAL_REPORT_SQL = "SELECT material_type.id as id, material_type.`no` as `no`, material_type.specification as specification, material_box.id AS box, material_box.row as row, material_box.col as col, material_box.height as height, SUM(material.remainder_quantity) AS quantity FROM material INNER JOIN material_box INNER JOIN material_type ON material.box = material_box.id AND material_type.id = material.type WHERE material_type.type = ? AND material.company_id = ? AND material_type.supplier = ? GROUP BY material.box, material.type ORDER BY material.box, material.type ASC";
	//获取贵重仓物料报表
