<template>
  <div class="tasks" v-loading="isLoading">
      ...
      <el-table-column label="操作" min-width="200">
        <template slot-scope="scope">
          ...
          <span style="margin-right:10px;cursor:pointer" title="强制解绑仓口"
                v-if="scope.row.state === 4 && (user.type === 1 || user.type === 3)"
                @click="forceUnbundlingWindow(scope.row)">
    ...
    <edit-status :is-edit-status.sync="isEditStatus" :edit-data="editData"
                 @showCheckTaskDetails="showCheckTaskDetails"></edit-status>
