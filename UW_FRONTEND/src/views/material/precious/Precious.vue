<template>
  <div class="precious" v-loading="isLoading">
...
<script>
  ...
  import {
    deletePreciousMaterialByIdsUrl,
    getOverdueMaterialUrl,
    materialCountUrl,
    supplierSelectUrl,
    updatePreciousMaterialTypeUrl
  ...
  export default {
    ...
    methods: {
      ...
      setFilter: function () {
        ...
        for (let i in copyUwMaterialInfo) {
          ...
          if (i === 'supplier') {
            ...
            filter = filter + ("#&#material_type." + i + "=" + copyUwMaterialInfo[i]);
          } else {
            filter = filter + ("#&#" + i + "like" + copyUwMaterialInfo[i]);
          }
          //}
      ...
      initForm: function () {
        Object.assign(this.$data.uwMaterialInfo, this.$options.data().uwMaterialInfo);
        this.filter = '';
      },
      ...
      deleteById: function () {
        ...
        if (!this.isPending) {
          this.isPending = true;
          let options = {
            url: deletePreciousMaterialByIdsUrl,
            data: {
              ids: filter
            }
          };
