<template>
  <div class="precious-details" v-loading="isLoading">
    <el-form :inline="true" class="precious-details-form">
    ...
    <el-table
      ...
      <el-table-column type="expand">
        <template slot-scope="props">
          <precious-details-item :row="props.row"></precious-details-item>
        </template>
      </el-table-column>
      <el-table-column
        label="序号"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span
            :class="{highLight:unInventoryData.includes(String(scope.row.material_type_id))}">{{scope.row.no}}</span>
        </template>
      </el-table-column>
      <el-table-column
        min-width="120"
...
<script>
  import {
    checkPreciousTaskUrl,
    coverPreciousUWMaterialOneKeyUrl,
    exportUwReportUrl,
  ...
  import PreciousDetailsItem from "./comp/PreciousDetailsItem";
  import {mapGetters, mapActions} from 'vuex'

  export default {
    name: "PreciousDetails",
    components: {PreciousDetailsItem},
    data() {
      return {
        tableData: [],
        isPending: false,
    ...
    methods: {
      ...
      coverUwMaterial: function () {
        ...
        if (!this.isPending) {
          ...
          this.isLoading = true;
          let options = {
            url: coverPreciousUWMaterialOneKeyUrl,
            data: {
              taskId: this.taskId
            }
          };
      ...
      finishTask: function () {
        if (!this.isPending) {
          this.isPending = true;
          let options = {
            url: finishInventoryPreciousTaskUrl,
            data: {
              taskId: this.taskId
            }
          };
