  ...
  <div class="precious-now">
    ...
    <div class="table-content">
      ...
      <el-table
          ...
          :data="tableData"
          :row-class-name="setHighlightRow"
          style="width:100%">
        <el-table-column
            prop="no"
            label="料号"
        ...
        <el-table-column
            prop="lackQuantity"
            :label="windowType === '1'?'本次欠发':'本次欠入/超入数量'">
        </el-table-column>
        <el-table-column
            prop="designator"
            label="位号">
    ...
    <out-task-details :is-out-show.sync="isOutShow" :task-item="taskItem" :finishType.sync="finishType"
                      @refreshData="refreshData"></out-task-details>
    <in-task-details :is-in-show.sync="isInShow" :task-item="taskItem" :finishType.sync="finishType"
                     @refreshData="refreshData"></in-task-details>
  </div>
</template>
    ...
    methods: {
      ...
      scannerHandler: function () {
        ...
        for (let i = 0; i < this.tableData.length; i++) {
          if (text.toLocaleUpperCase() === this.tableData[i].no.toLocaleUpperCase()) {
            ...
            //截料
            if (this.windowType === '1' && this.isPreciousCut === true/* && this.isOutShow === true*/) {
              this.backAfterCutting(packingListItemId, tempArray, i);
              return;
            }
            //出库
            if (this.windowType === '1' && this.isPreciousCut === false/* && this.isOutShow === true*/) {
              this.outM(packingListItemId, tempArray, i);
              return;
            }
            //入库
            if (this.windowType === '0' /*this.isInShow === true*/) {
      ...
      finish() {
        ...
        if (!this.isPending) {
          this.isPending = true;
          let options = {
            url: finishAllPreciousTaskItemUrl,
            data: {
              taskId: this.taskId
            }
          };
      ...
      setHighlightRow({row, index}) {
        if (this.highlightMaterial === row.no) {
