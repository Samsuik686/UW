    ...
    <el-dialog
      ...
      <div class="task-item">
        <div class="item">
          ...
          <div class="delete-all" style="margin-top: auto">
            <el-button
                @click="deleteAllMaterialRecord"
                size="small"
                type="danger">删除所有</el-button>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteAllMaterialRecord: function() {
        let _this = this;
        let deleteMaterial = function (id) {
          return new Promise((resolve, reject) => {
            let options = {
              url: taskDeletePreciousMaterialRecordUrl,
              data: {
                packingListItemId: _this.taskItem.packingListItemId,
                materialId: id
              }
            };
            axiosPost(options).then(response => {
        ...
        this.$confirm('你正在删除页面上所有扫描记录，请确认是否删除?', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning',
          center: false
          ...
          if (action === "confirm") {
            let promiseGroup = [];
            this.taskItem.infos.forEach(item => {
              promiseGroup.push(deleteMaterial(item.materialId))
            });
            Promise.all(promiseGroup).then(() => {
              this.$alertSuccess("删除成功");
              this.$emit('refreshData', true);
            }).catch((err) => {
              this.$alertWarning('删除存在错误');
              this.$emit('refreshData', true);
            })
          }
        }).catch(() => {});
